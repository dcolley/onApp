...
var OutageCtrl = function( $state, $scope, $rootScope, $localStorage, $stateParams, API, logSvc, $http, limitToFilter, filterFilter ) {

	$scope.$storage = $rootScope.$localStorage; // = $localStorage.$default({
	// $scope.data = $rootScope.data;
	// $scope.params = $stateParams;
	$scope.search = $scope.$storage.search.outage;

	$scope.$watch('search', function(search) {
		$scope.$storage.search.outage = search;
	});
	...
	switch( $scope.data.action ) {
		case "List":
			break;
		case "Add":
			$scope.outage = new API.outage();
			break;
		case "View":
			API.outage.get( { id: $stateParams.id } ).$promise.then( function( model, err ) {
	...
	$scope.refreshList = function() {
		...
		//	$scope.$storage.issues = $scope.project.issues({ pid: $scope.project.id} );
			$scope.$storage.outages = API.outage.list();
			// logSvc.trace('finished refresh');
		} catch(e) {
			logSvc.error( e );
			alert(e);
...
angular.module('onApp').controller( 'OutageCtrl', ['$state', '$scope', '$rootScope', '$localStorage', 
	'$stateParams', 'API', 'logSvc', '$http', 'limitToFilter', 'filterFilter', OutageCtrl] ); 
