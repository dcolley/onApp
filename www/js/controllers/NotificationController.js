...
var NotificationCtrl = function( $state, $scope, $rootScope, $localStorage, $stateParams, API, logSvc, $http, limitToFilter, filterFilter ) {
	...
	$scope.$watch('search', function(search) {
		$scope.$storage.search.notification = search;
	});
	...
	switch( $scope.data.action ) {
		case "List":
			break;
		case "Add":
			$scope.model = new API.notification();
			break;
		case "View":
			API.notification.get( { id: $stateParams.id } ).$promise.then( function( model, err ) {
	...
	$scope.loadMore = function() {
		API.notification.list({
			...
			if( res['count'] ) {
				angular.forEach( res['notification'], function( value, key ) {
					console.log('pushing');
					$scope.$storage.notifications.push( value );
				}) ;
			};
...
angular.module('onApp').controller( 'NotificationCtrl', ['$state', '$scope', '$rootScope', '$localStorage', 
	'$stateParams', 'API', 'logSvc', '$http', 'limitToFilter', 'filterFilter', NotificationCtrl] ); 
